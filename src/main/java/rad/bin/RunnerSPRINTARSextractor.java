...
public class RunnerSPRINTARSextractor extends RunnerNCEPextractor {
	...
	private double convertLat(double traceLat){
		...
		// Если значение меньше нуля, прибавляем 360, чтобы получить целое положительное
		if (curLat < 0) {
			curLat = 360 + curLat;
		}
	...
	public static long Calendar2YearHours(GregorianCalendar ccn) {
		// FIXME Правильно ли пересчитывает количество часов от начала года
		// почемуто в sprint_dat неправильное время
	...
	/**
	 * Конструирование имени файла SPRINTARS
	 * 
	 * @param year
	 *            год за который извлекаются данные
	 * @param prefix
	 *            префикс имени в соответствии с основной переменной, например
	 *            alfac или ps и др.
