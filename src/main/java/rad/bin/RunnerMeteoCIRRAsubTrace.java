	...
	void runModule(Module module) throws Exception {
		try {
			...
			if (getParStringByText("meteo_model_name").equalsIgnoreCase("cira")) {
				Double[] altModel = Hmodel.get("geom_height"); // извлечение
				// массива высот
				Double[] pressModelmbar = Hmodel.get("pressure"); // извлечение
				// массива
				// давлений
				// в mbar
				Double[] pressModelatm = new Double[pressModelmbar.length]; // подготовка
				// массива
				// давлений
				// в
				// atm
				...
				// смысл следующих шести строк - найти столбцы в файле CIRA
				// которые лежат слева и справа от заданной широты
				...
				// затем производится линейная интерполяция между значениями
				// ограничивающих столбцов
				// на текущую широту
				...
				// заполнение вектора массивами с высотой, давлением и
				// температурой
				// и запись в cira.dat
