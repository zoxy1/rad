...
public class RunnerRatScatter extends Runner {
	static Logger localLogger = Logger.getLogger(RunnerRatScatter.class);
	@Override
	void runModule(Module modul) throws Exception {
		...
		BpiOnlyMol = AtmModel.get("bpi_molecular");
		//AlphaOnlyMol = AtmModel.get("alpha_molecular");
		
		//Double TAUOnlyMol[] = AtmModel.get("tau_molecular"); //countTau(hsig ,AlphaOnlyMol); //new Double[Psig.length];
		
		Double TransmittOnlyMolSig[] = AtmModel.get("transmittance_molecular");  //countTransmittance(TAUOnlyMol);
